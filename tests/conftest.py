...
from datamodels.schemas import schema_contract, schema_customer, schema_income
...
def create_customers(db):
    ...
    crud_customer.create(session, schema_customer.CustomerBase(name='Bar'))
    crud_customer.create(session, schema_customer.CustomerBase(name='Bar2'))
    crud_customer.create(session, schema_customer.CustomerBase(name='Bar3'))
    crud_customer.create(session, schema_customer.CustomerBase(name='Bar4'))
    result = crud_customer.create(
        session,
        schema_customer.CustomerBase(name='FooBar3')
        )
...
def create_contract(db):
    ...
    logger.debug('fixture started')
    crud_contract.create(session, schema_contract.ContractBase(
        customer_id=1,
        rate=20,
    ...
    result = crud_contract.create(session, schema_contract.ContractBase(
        customer_id=3,
        rate=30,
        unit=Unit.DAY.value,
    ...
    logger.debug(f'fixture result from contract 4 => {result.id}')
...
def create_income(db):
    ...
    logger.debug('fixture started')
    crud_income.create(session, schema_income.IncomeBase(
        total=1,
    ...
    result = crud_income.create(session, schema_income.IncomeBase(
        total=3,
